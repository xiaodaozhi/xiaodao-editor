...
> 状态：**阶段一至九 — 已实现**（全部内置块类型（含图片块及上传管线、表格块、分割线、目录块）、行内标记（含链接 mark 与安全净化管线）、块级属性、slash 菜单、输入规则、悬停工具栏、拖拽手柄、剪贴板、国际化、主题、**原生 Markdown ↔ DocState 导入/导出、可控制尺寸的编辑器画布（`width`/`height` prop + 内部滚动）、常驻 FixedToolbar（`toolbarPosition: auto|top|bottom`，菜单向下弹出）、移动端长按跨块文本选择叠层、`build:demo` 演示产物构建命令**）
  ...
  view/                         # Vue 专属桥接 + 组件
    ...
    ui/                         # 浮动 UI 组件(全部 Teleport 到 <body>)
      ...
      FixedToolbar.vue            # 常驻顶/底操作栏（内嵌 HoverToolbar + plus/handle 按钮）。
                                  # 通过 toolbarPosition prop 控制位置：'auto'(桌面顶/移动端底)、'top'、'bottom'。
